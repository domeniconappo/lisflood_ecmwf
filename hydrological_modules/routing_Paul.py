    ...
    def initialSecond(self):
        ...
        if option['SplitRouting']:
            ...
            if not(option['InitLisflood']):
                ...
                # CM no longer necessary: if discharge in second line of routing is small, second line is NOT activated
                # CM see code for second line of routing

                #CMmod
                ##### self.var.Chan2M3Kin = self.var.CrossSection2Area * self.var.ChanLengthC + self.var.Chan2M3Start
                self.var.Chan2M3Kin = self.var.CrossSection2Area * self.var.ChanLengthC
                # CM Initializing volume of water in second line of routing

                #CMmod
                ##### self.var.ChanM3Kin = self.var.ChanM3 - self.var.Chan2M3Kin + self.var.Chan2M3Start
                self.var.ChanM3Kin = self.var.ChanM3 - self.var.Chan2M3Kin
                # CM Initializing volume of water in first line of routing
...
# --------------------------------------------------------------------------
    ...
    def dynamic(self, NoRoutingExecuted):
        ...
        if not(option['dynamicWave']):
            ...
            # Total sideflow to channel for DtRouting step
            ...
            if option['InitLisflood'] or (not(option['SplitRouting'])):
                ...
                #  ---- Single Routing ---------------
                # No split routing
#                SideflowChan[np.isnan(SideflowChan)] = 0 # TEMPORARY FIX FOR NAN IN LAKE OUTFLOW AT FIRST TIME STEP
                ...
                #CM  Total channel flow [cu m], equal to ChanM3Kin
                # at single kin. ChanQ is the same

                self.var.sumDisDay+=self.var.ChanQ
                # CM Cumulative channel flow on routing steps
                ...
                # CM mod ######################################################################
                TotalM3 = self.var.ChanM3Kin + self.var.Chan2M3Kin
                # CM total volume of water in channel, both routing line 1 and 2

                # self.var.ChanM3Kin = np.where(TotalM3 > self.var.M3Limit, self.var.M3Limit, TotalM3)
                self.var.ChanM3Kin = np.where((TotalM3 - self.var.M3Limit) > 1e-1, self.var.M3Limit, TotalM3)
                # CM Volume in first line of routing (bankful)
                # CM Avoid activating second line of routing if volume over bankful is too small
                # CM Minimum volume might need to be changed

                self.var.Chan2M3Kin = TotalM3 - self.var.ChanM3Kin
                # CM Volume in second line of routing (floodplains)
                ###############################################################################
                ...
                # CM Divide sideflow between routing line 1 and 2 according to relative volume of water
                ...
                #CMmod
                #self.var.Sideflow1Chan = np.where(self.var.ChanM3Kin > self.var.M3Limit, SideflowRatio*SideflowChan, SideflowChan) #original
                # CM this was causing instability problems, switching routing line 2 on and off between routing sub-steps
                # CM this has been removed because through computing sub-steps it could happen self.var.ChanM3Kin < self.var.M3Limit with a lot of water in routing line 2

                self.var.Sideflow1Chan = SideflowRatio*SideflowChan
                # CM Sideflow assigned to routing line 1 (bankful)

                #CM mod
                ##### self.var.Sideflow1Chan = np.where(np.abs(SideflowChan) < 1e-7, SideflowChan, self.var.Sideflow1Chan) #original
                # too small values for Sideflow2Chan are avoided
                self.var.Sideflow1Chan = np.where((SideflowChan - self.var.Sideflow1Chan) < 1e-7, SideflowChan, self.var.Sideflow1Chan)
                Sideflow2Chan = SideflowChan - self.var.Sideflow1Chan
                # CM Not sure this is still necessary
                ...
                # CM Flow from second line of routing
                # Superposition Kinematic
                # second line of routing

                # CM
                ##### self.var.ChanQ = np.maximum(self.var.ChanQKin + self.var.Chan2QKin - self.var.QLimit, 0)  # originale
                self.var.ChanQ = np.maximum(self.var.ChanQKin + self.var.Chan2QKin, 0)
                # CM Total channel flow [cu m], equal to sum of routing line 1 and 2
