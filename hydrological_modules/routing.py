...
# ************************************************************
        ...
        # # CM testing multipliers for channels geometry based on uparea
        # upstreamArea = compressArray(self.var.UpArea)
        # self.var.ChanBottomWidth = np.where(upstreamArea >= 15000000000, loadmap('ChanBottomWidth') * loadmap('ChanBottomWMult'), loadmap('ChanBottomWidth'))
        # ChanDepthThreshold = np.where(upstreamArea >= 15000000000, loadmap('ChanDepthThreshold') * loadmap('ChanDepthTMult'), loadmap('ChanDepthThreshold'))
        # ChanSdXdY = np.where(upstreamArea >= 15000000000, loadmap('ChanSdXdY') * loadmap('ChanSMult'), loadmap('ChanSdXdY'))
