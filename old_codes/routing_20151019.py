        ...
        if not(option['dynamicWave']):
            ...
            if option['InitLisflood'] or (not(option['SplitRouting'])):
              ...
              #  self.var.ChanQ = maxpcr(self.var.ChanQKin, null)
                ...
                # check if negative sideflow to channel1 is compensated by positive sideflow to channel2
                # and results in an overall sum of 0 -> in this case sideflow1 = sideflow

                Sideflow2Chan = SideflowChan - self.var.Sideflow1Chan
                # Sideflow2Chan=if(Sideflow2Chan lt 0,0,Sideflow2Chan)
                # to check if it work with negative sideflow e.g. irigation
                ...
                self.var.ChanQKin = (self.var.ChanM3Kin*self.var.InvChanLength*self.var.InvChannelAlpha)**(self.var.InvBeta)
                #self.var.ChanQKin = (self.var.ChanM3Kin/self.var.ChanLength/self.var.ChannelAlpha)**(1/self.var.Beta)
                ...
                #self.var.Chan2QKin = (self.var.Chan2M3Kin/self.var.ChanLength/self.var.ChannelAlpha2)**(1/self.var.Beta)
